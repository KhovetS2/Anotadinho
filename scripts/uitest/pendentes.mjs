...
pendente("teclado", "hjkl move igual às setas", async (bridge, ctx) => {
...
pendente("teclado", "abrir página de dentro de um embed não prende o foco na barra superior",
  async (bridge, ctx) => {
...
pendente("teclado", "dois Escapes sobem dois níveis", PAGINA_CONSULTA, async (bridge, ctx) => {
  await bridge.js(TECLA("ArrowDown"));
  await PAUSA(400);
...
pendente("vim", "modo visual seleciona por caractere",
  { md: "---\ntitle: __uitest\n---\nabcdef\n", vim: true },
  async (bridge, ctx) => {
    await bridge.js(`(() => {
...
pendente("vim", "modo visual-block seleciona um retângulo",
  { md: "---\ntitle: __uitest\n---\nabc\ndef\nghi\n", vim: true },
  async (bridge, ctx) => {
...
pendente("vim", "`/` abre o modo de comando fora da edição",
  ...
  async (bridge, ctx) => {
    ...
    const abriu = await bridge.js(`(() => {
      ...
      return modo.includes('COMANDO') || !!document.querySelector('.vim-comando, .editor__comando');
    })()`);
    ctx.assertEq(abriu, true, "`/` não abriu o modo de comando");
  });

pendente("vim", "com vim ligado há atalho próprio pro modo de navegação",
  { md: RASCUNHO, vim: true },
  async (bridge, ctx) => {
